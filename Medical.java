...
{
    JLabel l1=new JLabel("Equipment_Id");
    JLabel l2=new JLabel("Equipment_name");
    JLabel l3=new JLabel("Equipment_cost");
    JLabel l4=new JLabel("Equipment_expiry");
    ...
    public void actionPerformed(ActionEvent ae)
    {
        if(ae.getSource()==btn1)
        {
            try
            {
                st.execute("insert into Medical_Equipment values ('"+t1.getText()+"','"+t2.getText()+"','"+t3.getText()+"','"+t4.getText()+"')");
                t1.setText("");
                t2.setText("");
                t3.setText("");
                t4.setText("");
        ...
        if(ae.getSource()==btn2)
        {
            try
            {
                ResultSet rs = st.executeQuery("Select * from Medical_Equipment where Equipment_Id="+t1.getText());
                if(rs.next()){
        ...
        if(ae.getSource()==btn3)
        {
            try
            {
                st.execute("Delete from Medical_Equipment where Equipment_Id="+t1.getText());
                t1.setText("");
                t2.setText("");
                t3.setText("");
                t4.setText("");
        ...
        if(ae.getSource()==btn4)
        {
            try
            {
                st.execute("Update Medical_Equipment set Equipment_name='"+t2.getText()+"', Equipment_cost='"+t3.getText()+"', Equipment_expiry='"+t4.getText()+"', where Equipment_Id="+t1.getText());
                t1.setText("");
                t2.setText("");
                t3.setText("");
                t4.setText("");
