...
import javax.swing.*;
class Attire extends JFrame implements ActionListener
{
    ...
    public void actionPerformed(ActionEvent ae)
    {
        if(ae.getSource()==btn1)
        {
            try
            {
                st.execute("insert into Attire_Items values ('"+t1.getText()+"','"+t2.getText()+"','"+t3.getText()+"','"+t4.getText()+"')");
                t1.setText("");
                t2.setText("");
                t3.setText("");
                t4.setText("");
        ...
        if(ae.getSource()==btn2)
        {
            try
            {
                ResultSet rs = st.executeQuery("Select * from Attire_Items where Item_Id="+t1.getText());
                if(rs.next()){
        ...
        if(ae.getSource()==btn3)
        {
            try
            {
                st.execute("Delete from Attire_Items where Item_Id="+t1.getText());
                t1.setText("");
                t2.setText("");
                t3.setText("");
                t4.setText("");
        ...
        if(ae.getSource()==btn4)
        {
            try
            {
                st.execute("Update Attire_Items set Item_name='"+t2.getText()+"', Item_cost='"+t3.getText()+"', Item_expiry='"+t4.getText()+"', where Item_Id="+t1.getText());
                t1.setText("");
                t2.setText("");
                t3.setText("");
                t4.setText("");
